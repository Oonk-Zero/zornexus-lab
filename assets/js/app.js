...
(function () {
  ...
  function refreshDashboardStats() {
    // Guard: only run on the dashboard page where #stat-pending exists.
    // #nav-pending-count is present in the navbar on EVERY page, so it cannot
    // be used as a page discriminator. Using it caused this function to fire a
    // concurrent API call on every page, including pending.html — which raced
    // with pending.js's own API.getPendingList() call and broke the queue load.
    // On pending.html the nav badge is updated by pending.js updateCounts().
    // On upload.html the badge shows "—" (acceptable — no queue context there).
    var statPending = document.getElementById("stat-pending");
    if (!statPending) return;

    var badge = document.getElementById("nav-pending-count");

    // Use limit=1 — we only need the `total` field, not the item list.
    // The backend always returns the real total regardless of limit.
    API.getPendingList(1)
      ...
      .catch(function () {
        // silently fail — elements retain their current value
      });
  }
