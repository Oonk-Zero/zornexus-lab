...
const API = (() => {
   ...
   * }
   *
   * @param {number} [limit]  Optional item limit (backend default 50). Pass 1 when only total is needed.
   * @param {string} [search] Optional search filter.
   * @returns {Promise<object>}
   */
  function getPendingList(limit, search) {
    var url   = endpoint("pendingList");
    var params = [];
    if (limit  != null) params.push("limit="  + encodeURIComponent(limit));
    if (search != null && search !== "") params.push("search=" + encodeURIComponent(search));
